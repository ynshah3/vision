...
# Define the range for region_idx and lesion_iters
for region_idx in {0..3}; do
  for lesion_iters in {0..39}; do
    # Create directory
    dir="cornet_s_${region_idx}_${lesion_iters}_4096"
    mkdir -p "$dir"

    # Create files
    touch "${dir}/__init__.py" "${dir}/model.py" "${dir}/setup.py" "${dir}/test.py"

    # Populate __init__.py
    echo """from brainscore_vision import model_registry
from brainscore_vision.model_helpers.brain_transformation import ModelCommitment
from .model import get_model, get_layers

model_registry['cornet_s_${region_idx}_${lesion_iters}_4096'] = lambda: ModelCommitment(identifier='cornet_s_${region_idx}_${lesion_iters}_4096', activations_model=get_model('cornet_s_${region_idx}_${lesion_iters}_4096'), layers=get_layers('cornet_s_${region_idx}_${lesion_iters}_4096'))""" > "${dir}/__init__.py"
...
setup(
    ...
    # Populate model.py
    echo """from brainscore_vision.model_helpers.check_submission import check_models
import functools
...
def get_custom_cornet_s():
    ...
    url = f'https://storage.googleapis.com/neurodp/${region_idx}_${lesion_iters}_4096_ckpt.pt'
...
def get_model_list():
        return ['cornet_s_${region_idx}_${lesion_iters}_4096']


def get_model(name):
    assert name == 'cornet_s_${region_idx}_${lesion_iters}_4096'
    model = get_custom_cornet_s()
    preprocessing = functools.partial(load_preprocess_images, image_size=224)
    wrapper = PytorchWrapper(identifier='cornet_s_${region_idx}_${lesion_iters}_4096',
                            model=model,
...
if __name__ == '__main__':
    check_models.check_base_models(__name__)""" > "${dir}/model.py"
  done
done
